...
class H2OContext private (@transient val sparkContext: SparkContext, @transient conf: H2OConf) extends org.apache.spark.Logging
  ...
  def asRDD[A <: Product: TypeTag: ClassTag](fr : H2OFrame) : RDD[A] = toRDD[A](this, fr)

  /** Convert given H2O frame into DataFrame type */
  def asDataFrame(fr : H2OFrame)(implicit sqlContext: SQLContext) : DataFrame = toDataFrame(fr)
  def asDataFrame(fr: Frame)(implicit  sqlContext: SQLContext) : DataFrame = toDataFrame(new H2OFrame(fr))
  def asDataFrame(s : String)(implicit sqlContext: SQLContext) : DataFrame = toDataFrame(new H2OFrame(s))
